...
public class TestHelpers {
    ...
    public static void stubUntagDeliveryStreamWithProvidedOrEmptyResponse(AmazonWebServicesClientProxy proxy, UntagDeliveryStreamResponse response) {
        if (response == null) {
            response =  UntagDeliveryStreamResponse
                .builder()
                .build();
        }
        doReturn(response).when(proxy).injectCredentialsAndInvokeV2(any(
            UntagDeliveryStreamRequest.class),
            any());
    }

    public static void stubCreateDeliveryStreamWithProvidedOrEmptyResponse(AmazonWebServicesClientProxy proxy, CreateDeliveryStreamResponse response) {
        if (response == null) {
            response =  CreateDeliveryStreamResponse
                .builder()
                .build();
        }
        doReturn(response).when(proxy).injectCredentialsAndInvokeV2(any(
